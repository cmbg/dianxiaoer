(function ($) {
    ...
    function replyClick(el) {
        el.parent().parent().append("<div class='replybox'><textarea cols='80' rows='50'  placeholder='来说几句吧......' class=' content mytextarea' ></textarea><span  class='send'>发送</span></div>")
            .find(".send").click(function () {

            var content = $(this).prev().val();
            var gid = $('container').attr('name');
            var url = $('#url').attr('name');
            var pid = $(this).parent().parent().find('#pid').attr('name');
            var id = $(this).parent().parent().find('#pid').attr('title');
            var mpid = $(this).parent().parent().find('#mpid').attr('name');
            var mid = $(this).parent().parent().find('#mpid').attr('title');
            var vpid = $(this).parent().parent().find('#vpid').attr('name');

            $.ajax({
                url: url,
                type:'get',
                data: {'content': content, 'pid': pid,'id':id,'mpid':mpid,'mid':mid,'gid':gid, 'vpid':vpid},
