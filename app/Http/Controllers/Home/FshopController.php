...
class FshopController extends CommonController
{
    ...
    public function index(Request $request)
    {
        ...
        $uid = $user->uid;
        $fishpond = Fishpond::where('uid',$uid)->first()->toArray();
//        dd($fishpond);
        $status = Fishpond::where('uid',$uid)->first()->status;
//        dd($status);
        if(empty($fishpond)){
            return view('Home.Home_Fshop.Meikaitong');
        }

        if($status == 1){
            return view('Home.Home_Fshop.Houtaijinyong');
        }
